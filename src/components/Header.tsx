      ...
      <div className="relative min-w-0 pr-28 md:pr-0">
        ...
        {showMobileRecordButton ? (
          <button
            ...
            className="absolute right-0 top-0 inline-flex min-h-12 items-center justify-center gap-1.5 rounded-[14px] bg-[#0d66ff] px-4 py-3 text-sm font-black text-white shadow-[0_12px_24px_rgba(37,99,235,0.28)] transition hover:bg-blue-700 dark:bg-blue-500 dark:hover:bg-blue-400 lg:hidden"
          >
          ...
          {showRangeControls ? (
            <div
              className={`flex min-w-0 flex-wrap items-center gap-2 rounded-[14px] border border-[#d4e1f2] bg-white px-3 shadow-[0_10px_24px_rgba(30,86,180,0.12)] dark:border-white/[0.08] dark:bg-white/[0.04] sm:basis-auto sm:px-4 ${
                rangeMode === "weekday" ? "w-full flex-none py-2.5 sm:w-[390px] sm:py-2.5 xl:py-2" : "flex-[1_1_0] py-2.5 sm:py-3"
              }`}
            >
                ...
                <div className="grid flex-1 grid-cols-5 gap-1">
                  {WEEKDAY_FILTER_OPTIONS.map((day) => (
                    <button
                      ...
                      className={`rounded-[10px] px-2 py-1.5 text-xs font-black transition sm:px-3 sm:py-2 xl:py-1.5 ${
